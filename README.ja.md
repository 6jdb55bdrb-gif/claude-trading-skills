...
- **Lowcap Call Tracker**: APIキー不要で動作（パブリックFinVizスクリーナー + 無料のyfinance価格取得）。FINVIZ Eliteはスクリーニング高速化用（オプション）、`ANTHROPIC_API_KEY`は5役割のLLMレビュー用（オプション、未設定時は決定論的なオフライン採点にフォールバック、月間コスト上限あり）。`TELEGRAM_BOT_TOKEN`を設定すると実行サマリーをTelegramに通知し、`/stats`・`/open`・`/calls`などのコマンドにも応答（オプション）
