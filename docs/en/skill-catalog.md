...
| **[Lowcap Call Tracker]({{ '/en/skills/lowcap-call-tracker/' | relative_url }})** | Three FinViz variants for explosive US low caps and ETFs (squeeze, momentum breakout, ETF momentum), a five-role review (Researcher, Technician, Skeptic, Risk Manager, Judge) before any hit becomes a call, and a SQLite tracker that scores calls *and* the ones the Judge skipped — PnL, hit rate, per-role accuracy, weekly improvement proposals | <span class="badge badge-free">No API</span> <span class="badge badge-optional">FINVIZ Optional</span> <span class="badge badge-optional">Anthropic API Optional</span> |
