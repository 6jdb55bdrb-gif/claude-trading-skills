...
| **Lowcap Call Tracker** (`lowcap-call-tracker`) | Screen US low-cap stocks and ETFs for explosive moves, run every hit through a five-role review (Researcher, Technician, Skeptic, Risk Manager, Judge), track the resulting calls and shadow calls in SQLite with PnL, hit rate, and per-role accuracy, and push each run plus on-demand statistics to Telegram. | `finviz` optional, `anthropic_api` optional, `yfinance` **required**, `telegram` optional, `local_sqlite` **required**, `local_calculation` — | standalone | beta |
...
| **Lowcap Call Tracker** | ❌ Not used | 🟡 Optional | ❌ Not used | FINVIZ Elite speeds up screening (public HTML otherwise); `ANTHROPIC_API_KEY` optional for the five-role LLM review (a deterministic offline backend runs without it, and a monthly spend cap is enforced); prices via free yfinance; optional Telegram bot (`TELEGRAM_BOT_TOKEN`) pushes each run and answers /stats, /open, /calls from your phone |
